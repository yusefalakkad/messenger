...
/// Запись полноразмерного видео-сообщения (НЕ кружка).
/// Использует AVCaptureSession + AVCaptureMovieFileOutput.
/// • Жёсткий потолок длительности — 60 секунд
/// • Возвращает (url, duration) после остановки
@MainActor
final class VideoRecorder: NSObject, ObservableObject {
    ...
    func prepare(useFrontCamera: Bool = false) async {
    ...
    nonisolated private func configureSession(useFrontCamera: Bool) async {
        ...
        // Очищаем предыдущие inputs/outputs
    ...
    /// Запускает запись. Возвращает (url, duration) когда остановится. Nil — отмена/ошибка.
