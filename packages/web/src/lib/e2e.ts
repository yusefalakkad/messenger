...
/**
 * Глобальный флаг сквозного шифрования.
 *
 * false → «облачный» режим: сообщения хранятся на сервере и СИНХРОНИЗИРУЮТСЯ
 * между всеми устройствами пользователя (вошёл по коду — переписка на месте,
 * превью слева показывает текст). В пути защищено HTTPS. Сервер технически
 * может прочитать (как облачные чаты Telegram/Slack/Discord).
 *
 * Почему так: настоящее E2E + мультидевайс требует пароль, который знает ТОЛЬКО
 * пользователь (сервер — нет). При входе по СМС-коду такого секрета нет (код
 * знает сервер), поэтому device-bound E2E ломал синхронизацию между телефоном,
 * сайтом и десктопом. Выбран приоритет «работает везде».
 *
 * Чтобы вернуть строгий E2E — поставить true (нужен отдельный пароль-ключ).
 */
const E2E_ENABLED = false;

/**
 * Проверяет, включено ли E2E для данного чата.
 * При E2E_ENABLED=false всегда выключено → сообщения plaintext + синхрон везде.
 */
